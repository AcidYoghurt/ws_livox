...
def generate_launch_description():
    ...
    default_model_path = urdf_tutorial_path + '/urdf/robot/robot.urdf.xacro'
    default_world_path = urdf_tutorial_path + '/world/custom_room.world'
    default_rviz_path = urdf_tutorial_path + '/config/rviz/dispaly_model.rviz'
    ...
    # 获取文件内容生成新的参数
    robot_description = launch_ros.parameter_descriptions.ParameterValue(launch.substitutions.Command(['xacro ', launch.substitutions.LaunchConfiguration('model')]),value_type=str)
    ...
    # 启动 Gazebo 节点
    # 通过 IncludeLaunchDescription 包含另外一个 launch 文件（这里启动的是gazebo参数）
    ...
    # 启动 spawn_entity 节点，用于将urdf格式的模型转换为sdf格式（因为gazebo只能显示sdf格式的模型）
    ...
    return launch.LaunchDescription([
        ...
        # 事件动作，load_robot_diff_drive_controller
        launch.actions.RegisterEventHandler(
        event_handler=launch.event_handlers.OnProcessExit(
